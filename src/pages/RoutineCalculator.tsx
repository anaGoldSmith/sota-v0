        ...
        <TableCell className="px-2">
          {element.type === 'Steps' ? (
            ...
            <span className="text-sm font-medium text-foreground">
              {(() => {
                const details = (element.originalData as any)?.acroDetails as Array<{ name: string }> | undefined;
                if (details && details.length > 0) {
                  const shown = details.slice(0, 2).map(d => d.name).join(' + ');
                  const remaining = details.length - 2;
                  return remaining > 0 ? <>{shown} <span className="text-muted-foreground">(+{remaining} more)</span></> : shown;
                }
                return element.dbData?.name || 'Acrobatic Element';
              })()}
            </span>
            ...
            <div className="space-y-2">
              ...
              <Select defaultValue="fig-cop-2025-2028-individual" disabled={isViewMode}>
                <SelectTrigger id="rulebook">
                  <SelectValue placeholder="Select rulebook">FIG CoP 2025-2028 - Individual</SelectValue>
                </SelectTrigger>
                <SelectContent>
                  <SelectGroup>
                    <SelectLabel>FIG CoP 2025-2028</SelectLabel>
                    <SelectItem value="fig-cop-2025-2028-individual" className="pl-12">Individual</SelectItem>
                    <div className="relative flex w-full select-none items-center rounded-sm py-1.5 pl-12 pr-2 text-sm opacity-40 cursor-not-allowed">Group (coming soon)</div>
                  </SelectGroup>
                  <SelectSeparator />
                                      ...
                                      <tbody>
                                        {isPaired ? (
                                          ...
                                          <>
                                            {/* Base element row */}
                                            <tr className="border-b border-border/30">
                                              <td className="py-2 px-4 whitespace-nowrap"><Badge variant="outline" className="bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 border-green-300 text-[10px]">Base</Badge></td>
                                              <td className="py-2 px-4">
