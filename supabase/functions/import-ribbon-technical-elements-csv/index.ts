...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('📥 Parsing ribbon technical elements CSV content...');
    ...
    console.log(`✅ Parsed ${technicalElements.length} ribbon technical elements from CSV`);

    console.log('🗑️  Deleting existing ribbon technical elements...');
    ...
    if (deleteError) {
      ...
      throw new Error(`Failed to delete existing ribbon technical elements: ${deleteError.message}`);
    }

    console.log('📝 Inserting new ribbon technical elements...');
    ...
    if (insertError) {
      ...
      throw new Error(`Failed to insert ribbon technical elements: ${insertError.message}`);
    }

    console.log(`✅ Successfully imported ${insertedData.length} ribbon technical elements`);

    return new Response(
      JSON.stringify({
        success: true,
        message: `Successfully imported ${insertedData.length} ribbon technical elements`,
