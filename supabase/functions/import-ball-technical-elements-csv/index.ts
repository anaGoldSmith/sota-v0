...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('📥 Parsing ball technical elements CSV content...');
    ...
    console.log(`✅ Parsed ${technicalElements.length} ball technical elements from CSV`);

    console.log('🗑️  Deleting existing ball technical elements...');
    ...
    if (deleteError) {
      ...
      throw new Error(`Failed to delete existing ball technical elements: ${deleteError.message}`);
    }

    console.log('📝 Inserting new ball technical elements...');
    ...
    if (insertError) {
      ...
      throw new Error(`Failed to insert ball technical elements: ${insertError.message}`);
    }

    console.log(`✅ Successfully imported ${insertedData.length} ball technical elements`);

    return new Response(
      JSON.stringify({
        success: true,
        message: `Successfully imported ${insertedData.length} ball technical elements`,
