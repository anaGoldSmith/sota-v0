...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('📥 Parsing clubs technical elements CSV content...');
    ...
    console.log(`✅ Parsed ${technicalElements.length} clubs technical elements from CSV`);

    console.log('🗑️  Deleting existing clubs technical elements...');
    ...
    if (deleteError) {
      ...
      throw new Error(`Failed to delete existing clubs technical elements: ${deleteError.message}`);
    }

    console.log('📝 Inserting new clubs technical elements...');
    ...
    if (insertError) {
      ...
      throw new Error(`Failed to insert clubs technical elements: ${insertError.message}`);
    }

    console.log(`✅ Successfully imported ${insertedData.length} clubs technical elements`);

    return new Response(
      JSON.stringify({
        success: true,
        message: `Successfully imported ${insertedData.length} clubs technical elements`,
